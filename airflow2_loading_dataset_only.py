...
def list_files(bucket_name, prefix, processed_prefix='processed/'):
    ...
    if files:
        ...
        #Define the destination blob and create it with empty content
        destination_blob = bucket.blob(destination_object)
        destination_blob.upload_from_string('')

        #Use the rewrite method to copy the source blob to the destination
        source_blob.rewrite(destination_blob)
...
with DAG('food_orders_dag',
    ...
    beamtask = BeamRunPythonPipelineOperator(
        ...
        pipeline_options={
            ...
            "tempLocation": 'gs://food-orders-us/temp',
            "stagingLocation": 'gs://dataflow-bucket-us/staging',
        },
        py_options=[],
        py_interpreter="python3",
        py_system_site_packages=False,
        dataflow_config=DataflowConfiguration(
            job_name="food_orders_processing_job", project_id='food-orders-407014', location="us-central1"
        ),
    )
